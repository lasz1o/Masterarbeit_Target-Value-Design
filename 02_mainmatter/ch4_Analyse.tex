...
\label{sec:4.4.1}
Das primäre Ziel der Phase 2 ist die Überführung der funktionalen Anforderungen (aus Phase 0) in ein realisierungsfähiges Design, das Kostensicherheit bietet. Dieser Zustand wird als "Definitive Design" bezeichnet.
...
\end{itemize}
Ein essenzieller Bestandteil der Allokation ist das "Contingency Management". Das Budget wird nicht zu 100\% auf die Cluster verteilt. Ein Teil verbleibt als zentraler Puffer ("Project Contingency") beim Bauherrn/Steuerungsteam, um unvorhergesehene Risiken abzudecken, ohne die Target Cost zu erhöhen.\autocite[Vgl.][S.~43 ]{hill_target_2017}


\subsection{Organisatorische Struktur: Interdisziplinäre Cluster-Teams}
\label{sec:4.4.3}
Um die Komplexität des Gesamtprojekts handhabbar zu machen, wird die operative Projektorganisation in Phase 2 ähnlich wie zuvor das Kernteam in interdisziplinären "Cluster-Teams" (Cross-Functional Teams) strukturiert.\autocite[Vgl.][S.~59]{hill_target_2017}


\textbf{Zusammensetzung und Rollen}
Ein Cluster (z.B. "Hülle") vereint alle für dieses Teilsystem relevanten Kompetenzen an einem Tisch. Die typische Besetzung umfasst:\autocite[Vgl.][S.~64]{hill_target_2017}
...
Ein entscheidendes Novum des TVD-Prozesses ist die Integration der ausführenden Firmen ("Makers") bereits in dieser frühen Designphase. Ihre Rolle wandelt sich vom reinen Empfänger fertiger Pläne zum aktiven "Co-Creator". Sie bringen fertigungstechnisches Wissen ("Constructability") ein, um Lösungen zu finden, die funktional gleichwertig, aber günstiger zu fertigen sind.\autocite[Vgl.][S.~65]{hill_target_2017}



\subsection{Prozesslogik: Der zyklische Steuerungsloop (Continuous Estimating)}
\label{sec:4.4.4}
Das methodische Kernstück der Phase 2 ist das "Continuous Estimating". Um den im konventionellen Prozess üblichen "Blindflug" zwischen den Leistungsphasen zu vermeiden, wird der Planungsprozess in kurze, sich wiederholende Zyklen (Sprints) von typischerweise 3 bis 4 Wochen unterteilt.\autocite[Vgl.][S. 124 ]{hill_target_2017}
...
\label{sec:4.5.2}
Die Art der Kostensteuerung ändert sich fundamental. Während in Phase 2 noch aktiv kalkuliert und optimiert wurde ("Estimating"), geht es nun um das reine "Monitoring":\autocite[Vgl.][S. 105]{ballard_target_2025}
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Wissenssicherung:} Im besten Fall werden Abweichungen (sowohl positive als auch negative) analysiert, um die Kennzahlen für zukünftige Projekte ("Benchmarks" für Phase 0) zu schärfen.
\end{itemize}
